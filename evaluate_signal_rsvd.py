...
def find_files(result_path: str, window_size: int) -> list[str]:
    ...
    return glob(os.path.join(target_folder, '*__rsvd_difference.npz'))


def extract_data(file_path: str) -> (list[str], list[np.ndarray]):
    # load the file
    filet = np.load(file_path)
    filet2 = np.load(file_path.replace("rsvd_difference", "svd"))

    # get the interesting keys
    key_list = [key for key in filet.keys() if key.endswith('rnd_rank_5__sbsp_rep_2__add_rank_2__eigenvalues')]
    indices = []
    eigenvalues = []
    for key in key_list:
        ...
        # get the key to the original eigenvectors
        orig_key = "__".join((*key.split("__")[:-4], "eigenvalues"))
        orig_values = filet2[orig_key][:len(eigenvalue_array)]
        orig_values[orig_values < np.finfo(orig_values.dtype).eps] = np.finfo(orig_values.dtype).eps

        # compute difference to original eigenvalues and normalize eigenvalues
        eigenvalue_array = np.abs((eigenvalue_array-orig_values))/orig_values
...
def etl_eigenvalues(result_path: str = "result") -> list[str]:
    ...
    for window_length in window_sizes:
        ...
        name = os.path.join(result_path, f"RSVD_eigenvalues_window_{window_length}")
        names.append(name)
...
def plot_signals(df: pd.DataFrame, window_length: int):
    df = 1-df
    ax = df.boxplot(column=list(df.columns), return_type='axes', showfliers=False)
    ax.set_ylim([0.96, 1.01])
    ax.set_yscale('log')
    ax.set_title(f'Histograms of Eigenvalues Accuracy for {df.shape[0]} Hankel-Matrices\nwith window length {window_length}')
    plt.show()
