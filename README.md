...
published in IEEE Access (Open Access).

This accompanying repository aims to make the research and results from the paper reproducible.
While we are not aware of any imminent restrictions, we recommend setting up Python 3.9 or later (we used Python 3.11).

The code is mainly focused on the paper. If you are primarily interested in using the fast (IKA-)SST see the following:

> If you want to use our results for fast Change Point Detection, check out our pip-installable package [changepoynt](https://github.com/Lucew/changepoynt).
> The package contains all algorithms mentioned in the paper (activate our proposed modification with `fast_hankel=True`)

While the paper mainly covers square Hankel matrices for clarity, the algorithms work for rectangular matrices as well, and the 
...
We also provide a Jupyter Notebook: [MinimalExample.ipynb](MinimalExample.ipynb).
...
decompositions (FFT rSVD) using these functions, look at `randomized_hankel_svd_fft` and `irlb_fft` in 
...
CPU utilization! Numba will try to spawn as many threads as logical CPUs, which sometimes leads to lower performance.
...
below for information on how to rerun all our experiments. We saved intermediate results to reduce the reproducibility effort.
...
before cloning or downloading and place the files manually via the web interface.
...
necessary files to preprocess the repository. Adapt the path in the main function of 
...
## Speed factors
Implementation details (package versions, language, CPU) and available array/FFT libraries have secondary effects
(few ms) on how the asymptotic complexities transfer to runtime improvements varies.
...
Therefore, we thank all the countless contributors and initiators of Python and key libraries like numpy, 
...
We also modified code from [fbpca](https://github.com/facebookarchive/fbpca) licensed using the BSD license at the time
of publishing this code. Additionally, we want to express our gratitude to the developers of
[rocket-fft](https://github.com/styfenschaer/rocket-fft) licensed with a BSD3 license, which enabled us to use numba
with the FFT and significantly increase the speed of the algorithms.
